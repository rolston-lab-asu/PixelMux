"""
DIT "SWEEP" tab: the live Current-vs-Time transient plot plus a
live-metrics HUD.

DIT plots raw current (not density) and always auto-ranges
(transient timescale/magnitude vary far more between runs.
"""
...
class DITPlotPanel(Atom):
    ...
    _hud_charge = Typed(QLabel)
    _hud_peak_current = Typed(QLabel)
    ...
    def _build_plot_panel(self):
        ...
        self.plot_manager = PlotManager(
            ...
            y_label="Current", y_units="A",
        )
        self.plot_manager.enable_autorange()
        ...
        reset_view_btn.clicked.connect(self.plot_manager.enable_autorange)
    ...
    def _build_live_hud(self):
        ...
        self._hud_charge = make_metric("EXTRACTED CHARGE")
        self._hud_peak_current = make_metric("PEAK |CURRENT|")

        layout.addStretch()

        self._hud_abort = QPushButton("ABORT DIT")
    ...
    def reset_for_new_run(self):
        ...
        for lbl in (self._hud_charge, self._hud_peak_current):
            lbl.setText("--")
    ...
    def set_log_y(self, enabled):
        self.plot_manager.widget.setLogMode(False, enabled)

    def plot_curve(self, t, current, channel, loop_number):
        self.plot_manager.plot_curve(t, current, channel, loop_number)

    def set_active_pixel(self, pixel):
        self._hud_active_pixel.setText(f"Latest Pixel: {pixel}")

    def set_hud_metrics(self, charge_text, peak_current_text):
        self._hud_charge.setText(charge_text)
        self._hud_peak_current.setText(peak_current_text)
