"""
One full SPO hold run: validates inputs, starts/stops the SPOWorker,
routes its signals into the plot/results panels, and handles the
SPO-specific exports (per-pixel TXT, results table, PNG, CSV).
"""
import os
import time
...
class SPOController(QObject):
    ...
    def run_measurement(self):
        ...
        if self.worker is not None and self.worker.isRunning():
            self.log("ERROR: an SPO hold is already running")
            return
        ...
        spo_params = self.config_panel.get_spo_params()
        self.plot_panel.prepare_legends(selected, spo_params["loops"])
        ...
        # query the instruments from this GUI thread while the hold runs.
        self.worker = SPOWorker(self.inst.keithley, self.inst.relay, selected, spo_params)
    ...
    def _on_pixel_result(self, record):
        ...
        self.plot_panel.plot_curve(t, power_density, record["channel"], record["loop"])
        ...
        if self.config_panel.autosave_curves_enabled():
            ...
            try:
                _curve_path, curve_filename = self.exporter.save_curve_now_spo(record)
                saved_curve = True
        ...
        if self.config_panel.autosave_table_enabled():
            self.exporter.sample_name = self.get_sample_name() or DEFAULT_SAMPLE_NAME
            try:
                self.exporter.save_table_row_now_spo(record, curve_filename)
                saved_table = True
            except Exception as e:
    ...
    def _on_pixel_faulted(self, pixel, area, fault, loop_number):
        ...
        self.results_panel.add_result_row(
            pixel, area, None, None, fault, loop_number, row_token=id(fault_entry),
        )
    ...
    def _on_sweep_finished(self, aborted, had_error):
        ...
        if aborted:
            self.log("SPO aborted")
        elif had_error:
            self.log("SPO ended with an error")
        else:
            self.log("SPO complete")
        ...
        if self.results:
            ...
            if table_on and curves_on:
                self.log(
                    ...
                    f"written to {self.exporter.manifest_path_spo()}"
                )
            elif curves_on:
                self.log(
                ...
                self.log(
                    ...
                    f"to {self.exporter.manifest_path_spo()} (curves not auto-saved)"
                )
            else:
    ...
    def _update_path_preview(self, change=None):
        ...
        if table_on and curves_on:
            path = self.exporter.preview_txt_path_spo(pixel)
            ...
            path = self.exporter.manifest_path_spo()
            panel.set_path_preview(
    ...
    def save_results(self, auto=False):
        ...
        try:
            for record in self.results:
                self.exporter.save_curve_now_spo(record)
            self.log(f"Saved {len(self.results)} SPO text file(s) to {self.exporter.raw_curves_dir(create=False)}")
        except Exception as e:
    ...
    def export_plot_png(self):
        try:
            ...
            suggested_path = os.path.join(folder, f"{basename}_spo_{timestamp}.png")

            path, _ = QFileDialog.getSaveFileName(
                self.parent_widget, "Export SPO Curve Image", suggested_path, "PNG Image (*.png)"
            )
            if not path:
                return  # user cancelled
    ...
    def export_results_csv(self):
        ...
        try:
            ...
            timestamp = time.strftime("%H%M%S")
            basename = self.exporter._basename()
            suggested_path = os.path.join(folder, f"{basename}_spo_results_{timestamp}.csv")
